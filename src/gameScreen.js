...
export default class gameScreen extends Phaser.Scene {
  ...
  constructor() {
    ...
    this.screenState = 3; // 0: game, 1: camera 2: jumpscare 3: main menu 4: game over 5: win screen 6: loading screen
  ...
  update(time, delta) {
        ...
        this.drawChange = false;
        if (this.keyPlus.isDown && this.debug == true) {
...
//         // for timers that play when the game is paused
       ...
       switchScreenState(num)
        ...
        switch (num)
        {
            ...
            if (this.screenState == 6) this.screens[6].scene.switch('gameScreen');
            if (this.screenState == 7) this.screens[5].scene.switch('gameScreen');
            ...
            break;
            case 6:
            if (this.screenState == 3) this.screens[0].scene.switch('loadingScreen');
            break;
            case 7:
            let newspaperScreen = this.scene.get('newspaperScreen');
            newspaperScreen.changeTimer = new timer(11500);
            newspaperScreen.changeTimer.finishCallback = async () => {
                    newspaperScreen.blackRectangle.setAlpha(1);
                    this.switchScreenState(0);
            };
            newspaperScreen.changeTimer.Start();
            if (this.screenState == 6) this.screens[6].scene.switch('newspaperScreen');
            if (this.screenState == 3) this.screens[0].scene.switch('newspaperScreen');
            break;
        }
